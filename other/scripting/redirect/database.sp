...
stock database_UpdateServerInfos(const String:ip[], const String:hostname[], bool:private2, players, bots, maxplayers, String:map[], database_CallBack:externalCallBack=database_CallBack:-1, any:arg=0)
{
    if(g_bDatabase_FailState)
        return;
    ...
    FormatEx(query, sizeof(query), DATABASE_UPDATE_SERVER_INFO, hostname_Escaped, private2, players, bots, maxplayers, map, ip);
    database_EnqueueQuery(query, database_GeneralCallback, externalCallBack, arg);
    database_RunNextQuery();
}
