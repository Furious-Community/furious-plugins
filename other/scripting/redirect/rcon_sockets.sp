...
stock rcon_CommandQueue_GetCommand(Handle:queue, index, String:buffer[], size)
{
...
stock rcon_CommandQueue_BroadcastErrorCallback(rconIndex, Handle:queue, rcon_Errors:rconError, errorType, errorNum)
{
    ...
    {
        new id = rcon_CommandQueue_GetCommand(queue, index, buffer, sizeof(buffer));
...
stock rcon_GetSocketAddress(rconIndex, String:socketAddress[], size)
{
    GetArrayString(GetArrayCell(g_hRconList, _:rcon_StructureElement_ServerIp), rconIndex, socketAddress, size);
}

stock rcon_GetSocketIp(rconIndex, &port=0, String:socketIp[], size)
{
...
stock rcon_GetServerPassword(rconIndex, String:serverPassword[], size)
{
...
stock bool:rcon_GetCommandContext_Server(CommandUserId, String:buffer[], size)
{
    new rconIndex;
...
stock bool:rcon_GetCommandContext_Command(CommandUserId, String:buffer[], size)
{
    new rconIndex;
    new commandIndex;
...
stock rcon_GetNextCommand(rconIndex, &id, String:buffer[], size)
{
    new Handle:queue = rcon_GetCommandQueue(rconIndex);
...
stock rcon_PackString(String:str[], String:packet[], size)
{
...
stock rcon_GetErrorString(rcon_Errors:rconError, errorType, errorNum, String:buffer[], size)
{
