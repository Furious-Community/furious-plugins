...
eDatabase_ServerAdvertiseList_Info g_CurrentAdvertisedServerInfo;
...
public OnPluginStart()
{
    ...
    BuildServerIp(g_sServerIp, sizeof(g_sServerIp));
...
public OnMapEnd()
{
    ...
    if(g_bAutoCleanupDb)
        database_CleanupTables(CLEANUP_FOLLOW_TIMEOUT, CLEANUP_SERVERS_MIN_FAIL, CLEANUP_SERVERS_TIMEOUT, view_as<database_CallBack> (0), 0);
}
...
stock UpdateState()
{
    ...
    {
        database_UnRegisterServer(g_sServerExternalIp, view_as<database_CallBack> (0), 0);
    }
    else if(
        ...
        database_RegisterServer(g_sServerExternalIp, g_sServerQueryIp, g_sServerQueryIpForGroups, g_sServerShortName, g_sGameDir, g_sMyGroups, g_sShowInGroups, g_sExposePWForGroups, g_sAdvertiseInGroups, g_iAdvertiseMinPlayers, g_iAdvertiseMinSlots, g_iServerOrder, g_sServerPW, g_sServerRconPW, view_as<database_CallBack> (0), 0);
...
stock BuildServerIp(String:ip[], size)
{
...
{
    if(!StrEqual("", g_sMyGroups))
        database_GetServerAdvertiseList(g_sServerExternalIp, g_sMyGroups, g_sGameDir, g_iServerQueriesCoolDown, OnGetServerAdvertiseListCallBack, 0);
...
stock OnClientConnectServer(client, String:ip[])
{
    ...
    {
        database_SetFollowInfo(steamId, name, false, g_sServerExternalIp, ip, view_as<database_CallBack> (0), 0);
    }
...
public OnUserConfirmedCallBack(Handle:sqlResponse, any:clientId)
{
    ...
    if(SQL_GetAffectedRows(sqlResponse) > 0)
    {
        ...
        {
            database_GetFollowSourceServer(steamId, g_sMyGroups, OnFollowSourceServerRetrieveCallback, 0);
        }
    }
}
...
public OnFollowSourceServerRetrieveCallback(Handle:sqlResponse, any:arg)
{
    ...
    static eDatabase_FollowSourceServer_Info infos;
    
    if(!database_ExtractFollowSourceServerRow(sqlResponse, infos))
       return;
    
    if(StrEqual("", infos.eDatabase_FSS_RconPW))
       return;
    ...
    ReplaceString(infos.eDatabase_FSS_Name, sizeof(infos.eDatabase_FSS_Name), "\"", "'");
    
    decl String:command[500];
    Format(command, sizeof(command), "redirect_notify_confirm \"%s\" \"%s\" %d %d %d \"%s\" \"%s\"", 
                                    infos.eDatabase_FSS_Steam,
                                    infos.eDatabase_FSS_Name,
                                    players, bots, maxplayers,
                                    map,
                                    serverName);
                 
    rcon_SendCommand(infos.eDatabase_FSS_Server, infos.eDatabase_FSS_RconPW, command, .erCB = OnRconError);
}

public OnRconError(id, any:arg, rcon_Errors:rconError, errorType, errorNum)
{
    decl String:errorStr[200];
    rcon_GetErrorString(rconError, errorType, errorNum, errorStr, sizeof(errorStr));
...
public OnGetServerListCallBack(Handle:sqlResponse, any:clientId)
{
    ...
    {
        static eDatabase_ServerList_Info infos;
        decl String:itemId[100];
        ...
        if(infos.eDatabase_ServerList_UpToDate)
        {
            if(infos.eDatabase_ServerList_Players-infos.eDatabase_ServerList_Bots >= infos.eDatabase_ServerList_MaxPlayers)
                pp_UpdateItem(g_hClientPanels[client], itemId, hostname, ITEMDRAW_WITHNEXT | ITEMDRAW_DISABLED, false);
...
public OnGetServerAdvertiseListCallBack(Handle:sqlResponse, any:data)
{
    ...
    if(
        ...
        StrEqual("", g_CurrentAdvertisedServerInfo.eDatabase_Ip)
       )
    {
        #if defined DEBUG
    ...
    if(g_CurrentAdvertisedServerInfo.eDatabase_UpToDate)
    {
        #if defined DEBUG
            LogMessage("Server is up-to-date");
        #endif
        if(
            g_CurrentAdvertisedServerInfo.eDatabase_AdvMinPlayers > g_CurrentAdvertisedServerInfo.eDatabase_Players-g_CurrentAdvertisedServerInfo.eDatabase_Bots ||
            g_CurrentAdvertisedServerInfo.eDatabase_AdvMinSlots > (g_CurrentAdvertisedServerInfo.eDatabase_MaxPlayers - (g_CurrentAdvertisedServerInfo.eDatabase_Players-g_CurrentAdvertisedServerInfo.eDatabase_Bots))) 
        {
            #if defined DEBUG
                LogMessage("Not adevertising: Server min players = %s ; min slots: %d",  g_CurrentAdvertisedServerInfo.eDatabase_AdvMinPlayers, g_CurrentAdvertisedServerInfo.eDatabase_AdvMinSlots);
            #endif
            return;
        }
        ...
        if(!StrEqual("", g_CurrentAdvertisedServerInfo.eDatabase_ShortHostname))
            strcopy(hostname, sizeof(hostname), g_CurrentAdvertisedServerInfo.eDatabase_ShortHostname);
        else if(!StrEqual("", g_CurrentAdvertisedServerInfo.eDatabase_Hostname))
            strcopy(hostname, sizeof(hostname), g_CurrentAdvertisedServerInfo.eDatabase_Hostname);
        else
            strcopy(hostname, sizeof(hostname), g_CurrentAdvertisedServerInfo.eDatabase_Ip);
        ...
        if(g_bShortAdvertiseMessage)
             PrintToChatAll("%t", "Server short advertise", hostname, g_CurrentAdvertisedServerInfo.eDatabase_Map, g_CurrentAdvertisedServerInfo.eDatabase_Players-g_CurrentAdvertisedServerInfo.eDatabase_Bots, g_CurrentAdvertisedServerInfo.eDatabase_MaxPlayers);
        else
        {
            PrintToChatAll("%t", "Server advertise", hostname, g_CurrentAdvertisedServerInfo.eDatabase_Ip);
            PrintToChatAll("%t", "Server info", g_CurrentAdvertisedServerInfo.eDatabase_Map, g_CurrentAdvertisedServerInfo.eDatabase_Players-g_CurrentAdvertisedServerInfo.eDatabase_Bots, g_CurrentAdvertisedServerInfo.eDatabase_MaxPlayers);
        }
    ...
    {
        #if defined DEBUG
            LogMessage("Querying %s @\"%s\"", g_CurrentAdvertisedServerInfo.eDatabase_Ip, g_CurrentAdvertisedServerInfo.eDatabase_QueryIp);
        #endif
        a2s_queryServer(g_CurrentAdvertisedServerInfo.eDatabase_Ip, g_CurrentAdvertisedServerInfo.eDatabase_QueryIp, -1);
    }
}

stock AdvertiseOnA2SResults(a2s_infoStructure infos)
{
    if(infos.a2s_status != eA2S_Success)
    {
        database_IncreaseServerRefreshFailures(infos.a2s_ip, view_as<database_CallBack> (0), 0);
    }
    else
    {
        database_UpdateServerInfos(infos.a2s_ip, infos.a2s_name, infos.a2s_private, infos.a2s_players, infos.a2s_bots, infos.a2s_maxplayers, infos.a2s_map, view_as<database_CallBack> (0), 0);
        
        if(
            g_CurrentAdvertisedServerInfo.eDatabase_AdvMinPlayers <= infos.a2s_players-infos.a2s_bots && 
            g_CurrentAdvertisedServerInfo.eDatabase_AdvMinSlots <= (infos.a2s_maxplayers - (infos.a2s_players-infos.a2s_bots))
          )
        {
            decl String:hostname[100];
            if(!StrEqual("", g_CurrentAdvertisedServerInfo.eDatabase_ShortHostname))
...
public OnFollowListCallBack(Handle:sqlResponse, any:clientId)
{
    ...
    while(SQL_FetchRow(sqlResponse))
    {
        decl String:hostname[100];
        decl String:itemId[100];
        static eDatabase_FollowList_Info infos;
...
public a2s_callback(any:clientId, a2s_infoStructure infos)
{
    if(clientId == -1)
    {
...
public a2s_playerCallback(any:clientId, a2s_playersInfoStructure infos)
{
    new client = GetClientOfUserId(clientId);
