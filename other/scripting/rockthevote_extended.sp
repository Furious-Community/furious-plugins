...
AttemptRTV(client)
{
	...
	if (GetClientCountEx(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
...
int GetClientCountEx(bool ignoreSpectators)
{
	int cClient;
	for (int i = 1; i <= MaxClients; ++i)
	{
		if (!IsClientInGame(i) || IsFakeClient(i) || ignoreSpectators && GetClientTeam(i) == CS_TEAM_SPECTATOR)
			continue;
		++cClient;
	}
...
public void Event_PlayerTeam(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (team == CS_TEAM_SPECTATOR && oldTeam != CS_TEAM_SPECTATOR)
	{
		--g_Voters;
	}
	else if (team != CS_TEAM_SPECTATOR && oldTeam == CS_TEAM_SPECTATOR)
	{
		++g_Voters;
	}
}