...
#define SM_SKINCHOOSER_VERSION		"4.9"
...
public void OnPluginStart()
{
	...
	g_ForcePlayerSkinTimer = CreateConVar("sm_skinchooser_forceplayerskintimer" , "2.0", "Time in seconds when Model gets applied if forceplayerskin is set to 1!!!", _, true, 1.0, true, 100.0);	
	...
	g_SkinAdminTimer = CreateConVar("sm_skinchooser_skinadmintimer","2.0", "Time in seconds when Model gets applied if skinadmin is enabled!!!", _, true, 1.0, true, 100.0);		
...
public int Menu_Model(Handle menu, MenuAction action, int param1, int param2)
{
	...
	if(action == MenuAction_Select)
	{
		if (StrEqual(Game, "csgo") && GetConVarInt(g_arms_enabled) == 1)
		{
			CreateTimer(0.1 , CommandSecMenu , GetClientUserId(param1));			
		}
...
public Action CommandSecMenu(Handle timer, any data)
{
	int param1 = GetClientOfUserId(data);
	if (param1 == 0) return Plugin_Handled;
...
public Action Timer_Menu(Handle timer, any data)
{
	...
	{	
		if( GetConVarBool(g_autodisplay) )
		{
			...
			if( GetConVarBool(g_displaytimer))
			{
				...
				{
					CreateTimer(GetConVarFloat(g_menustarttime), Timer_Menu, GetClientUserId(client));
				}
			}
...
public Action Event_PlayerSpawn(Handle event,  const char[] name, bool dontBroadcast)
{
	...
	{	
		...
		if (StrEqual(Game, "csgo") && GetConVarInt(g_arms_enabled) == 1)
		{
			...
			{			
  				CreateTimer(1.5, PlayerModel, GetClientUserId(client));
  			}
  		}
  		else
  		{
		...
		if (!IsFakeClient(client) && IsValidClient(client) && GetConVarInt(g_SkinAdmin) == 1)	
		{
			...
			{			
				CreateTimer(GetConVarFloat(g_SkinAdminTimer), skin_admin, GetClientUserId(client));				
			}
		}
		...
		{		
			...
			if(admin == INVALID_ADMIN_ID)	
			{
				CreateTimer(GetConVarFloat(g_ForcePlayerSkinTimer), skin_players, GetClientUserId(client));	
			}
		}			
  	}
}

public Action PlayerModel(Handle timer, any data)
{
	int client = GetClientOfUserId(data);
...
public Action skin_players(Handle timer, any data)
{
	int client = GetClientOfUserId(data);
	if (client == 0) return Plugin_Continue;
...
public Action skin_admin(Handle timer, any data)
{
	int client = GetClientOfUserId(data);
	if (client == 0) return Plugin_Continue;
